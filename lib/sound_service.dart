...
class SoundService {
  ...
  static bool _isMuted = false;  // Изменено: был конфликт с геттером
  ...
  static Future<void> _playSound(String fileName, {double volume = 1.0}) async {
    if (_isMuted) return;
    if (_sfxPool.isEmpty) return; // Защита от пустого пула
  ...
  } // ← Закрывающая скобка для toggleMute была пропущена
  
  static Future<void> dispose() async {
    await _backgroundPlayer?.dispose();
    for (final p in _sfxPool) { // Исправлено: было "pin_sfxPool"
      await p.dispose();
    }
    _sfxPool.clear();
